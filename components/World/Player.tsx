...
export const Player: React.FC = () => {
  ...
  const { status, laneCount, takeDamage, hasDoubleJump, activateImmortality, isImmortalityActive, togglePause } = useStore();
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Always allow toggle pause if game is active (PLAYING or PAUSED)
      ...
      else if (e.key === 'Shift' || e.key === 'I' || e.key === 'i') {
          activateImmortality();
      }
    };
  ...
  }, [status, laneCount, hasDoubleJump, activateImmortality, togglePause]);
  ...
  useFrame((state, delta) => {
    ...
    // 1. Horizontal Position & Smoother Interpolation
    targetX.current = lane * LANE_WIDTH;
    // Use a slightly lower lerp factor for smoother, heavier feel, but high enough to be responsive
    ...
    // 3. Refined Banking Rotation
    // Calculate distance to target to determine how much to lean
    const distToTarget = targetX.current - groupRef.current.position.x;
    // Smooth banking based on distance, clamped to avoid over-rotation
    const targetRotation = -distToTarget * 0.35; 
    const clampedRotation = THREE.MathUtils.clamp(targetRotation, -0.5, 0.5);
