...
const LaneGuides: React.FC = () => {
    ...
    return (
        <group position={[0, 0.02, 0]}>
            ...
            {/* Lane Separators - Blue Glowing Lines */}
            {separators.map((x, i) => (
...
const RetroSun: React.FC = () => {
    ...
    const uniforms = useMemo(() => ({
        uTime: { value: 0 },
        uColorTop: { value: new THREE.Color('#ffff00') }, // Yellow
        uColorBottom: { value: new THREE.Color('#228b22') } // Forest Green
    }), []);
...
export const Environment: React.FC = () => {
  return (
    <>
      <color attach="background" args={['#051005']} /> {/* Dark Green BG */}
      <fog attach="fog" args={['#051005', 40, 160]} />
      
      <ambientLight intensity={0.3} color="#004400" />
      <directionalLight position={[0, 20, -10]} intensity={1.5} color="#ffd700" />
      <pointLight position={[0, 25, -150]} intensity={2} color="#32cd32" distance={200} decay={2} />
