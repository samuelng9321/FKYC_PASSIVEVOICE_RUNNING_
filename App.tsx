...
const CameraController = () => {
  ...
  const { laneCount, lastDamageTime } = useStore();
  ...
  useFrame((state, delta) => {
    ...
    const shakeDuration = 400; // ms
    if (timeSinceDamage < shakeDuration) {
         // Decay intensity over time
         const intensity = 0.4 * (1 - timeSinceDamage / shakeDuration);
         targetPos.x += (Math.random() - 0.5) * intensity;
         targetPos.y += (Math.random() - 0.5) * intensity;
         targetPos.z += (Math.random() - 0.5) * intensity;
    }
